...
看板不会创建 Codex 会话，也不会猜测某个会话是否使用了 Skill。建议先创建 3 个无 Skill 新会话，等它们全部完成后，再初始化 Project Brief 并创建 3 个有 Skill 新会话。每组使用相同的项目、模型、推理等级和理解要求，并且不要在测试会话中修改项目。

刷新看板后，在“无 Skill / 有 Skill 会话对比”中按对应顺序选择两组主会话。可以继续添加或删除会话，横轴会随会话数量变化。页面支持单次与累计折线图，并可切换总 token、输入、缓存输入、非缓存输入、输出和会话耗时。

初始化及后续维护会话可以作为有 Skill 方案的固定成本加入累计图。每个主会话的成本包含它启动的子代理。选择只保留在当前页面内，不写入文件、浏览器存储或 Codex 日志；刷新页面后需要重新选择。
